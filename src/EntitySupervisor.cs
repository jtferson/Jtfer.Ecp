...
namespace Unity.IL2CPP.CompilerServices {
    ...
    /// </summary>
    public interface ISupervisorEventListener {
        ...
        void OnWorldDestroyed (EntitySupervisor supervisor);
    }
#endif
    public class EntitySupervisor : IDisposable
    {
        /// <summary>
        /// Last created instance of EcsWorld.
        /// Can be force reassigned manually when multiple worlds in use.
        /// </summary>
        public static EntitySupervisor Active = null;

        /// <summary>
        /// List of all entities (their components).
        /// </summary>
        protected EntityInternal[] _entities = new EntityInternal[1024];
        ...
        /// </summary>
        protected EntityFilter[] _filters = new EntityFilter[64];

        protected int _filtersCount;

        /// <summary>
        /// List of entity owners
        /// </summary>
        protected EntityOwner[] _owners = new EntityOwner[8];

        protected int _ownersCount;

        /// <summary>
        /// List of requested one-frame component filters.
        /// </summary>
        protected readonly Dictionary<int, EntityFilter> _oneFrameFilters = new Dictionary<int, EntityFilter>(32);

        /// <summary>
        /// Temporary buffer for filter updates.
        /// </summary>
        readonly ComponentMask _delayedOpMask = new ComponentMask();
        ...
        /// </summary>
        readonly System.Collections.Generic.List<ISupervisorEventListener> _eventListeners = new System.Collections.Generic.List<ISupervisorEventListener> (4);

        /// <summary>
        /// Adds external event listener.
        /// </summary>
        /// <param name="listener">Event listener.</param>
        public void AddEventListener (ISupervisorEventListener listener) {
#if DEBUG
            if (listener == null) { throw new Exception ("Listener is null"); }
        ...
        /// </summary>
        /// <param name="listener">Event listener.</param>
        public void RemoveEventListener (ISupervisorEventListener listener) {
#if DEBUG
            if (listener == null) { throw new Exception ("Listener is null"); }
#endif
        ...
        /// </summary>
        public int CreateEntity(int pipeline, int entityIndex)
        {
            var entity = CreateEntityInternal(pipeline, entityIndex);
        ...
        /// </summary>
        public T CreateEntityWith<T>(int pipeline, int entityIndex) 
            where T : class, new()
        {
            T component;
            CreateEntityWith<T>(out component, pipeline, entityIndex);
            return component;
        }
        ...
        public int CreateEntityWith<T>(out T component, int pipeline, int entityIndex) 
            where T : class, new()
        {
            var entity = CreateEntityInternal(pipeline, entityIndex);
        ...
        /// <returns>New entity Id.</returns>
        public int CreateEntityWith<T1, T2>(out T1 c1, out T2 c2, int pipeline, int entityIndex) where T1 : class, new() where T2 : class, new()
        {
#if DEBUG
        ...
        /// <returns>New entity Id.</returns>
        public int CreateEntityWith<T1, T2, T3>(out T1 c1, out T2 c2, out T3 c3, int pipeline, int entityIndex) where T1 : class, new() where T2 : class, new() where T3 : class, new()
        {
#if DEBUG
        ...
        public T EnsureComponent<T>(int entity, out bool isNew) where T : class, new()
        {
...
#endif
            var entityData = _entities[entity];
            var pool = ComponentPool<T>.Instance;
            ...
            if (pool.IsOneFrame)
            {
                ...
                {
                    _oneFrameFilters[pool.TypeIndex] = GetFilter(typeof(AnyEntityType.EntityFilter<T>));
                }
            }
        ...
        public int GetComponents(int entity, ref object[] list)
        {
            ...
            {
                list[j] = Internals.EcpHelpers.ComponentPools[entityData.Components[i]].GetExistedItemById(entityData.Components[i + 1]);
            }
            return itemsCount;
        }
        ...
        public EcsWorldStats GetStats()
        {
            var stats = new EcsWorldStats()
            {
                ...
                Owners = _ownersCount,
                Components = Internals.EcpHelpers.ComponentPoolsCount,
        ...
        public void ProcessDelayedUpdates()
        {
            ...
            for (var i = 0; i < iMax; i++)
            {
                ...
                switch (op.Type)
                {
                    case DelayedUpdate.Op.CreateEntity:
                        UpdateOwners(op.Entity, false);
                        break;
                    case DelayedUpdate.Op.RemoveEntity:
                        ...
                        UpdateFilters(op.Entity, entityData.EntityTypeIndex, _delayedOpMask, entityData.Mask);
                        UpdateOwners(op.Entity, true);
                        for (int ii = 0, iiMax = componentsCount; ii < iiMax; ii += 2)
                        {
                            var poolId = entityData.Components[ii];
                            var pool = Internals.EcpHelpers.ComponentPools[poolId];
#if DEBUG
                            var dbgCmpt1 = pool.GetExistedItemById(entityData.Components[ii + 1]);
...
#endif
#if ECP_ENABLE_WORLD_EVENTS
                            var evtCmpt1 = pool.GetExistedItemById(entityData.Components[ii + 1]);
                    ...
                    case DelayedUpdate.Op.RemoveComponent:
                        ...
                        var dbgCmpt2 = op.Pool.GetExistedItemById(op.ComponentId);
                        for (int ii = 0, iiMax = _debugListeners.Count; ii < iiMax; ii++)
                        {
...
#endif
#if ECP_ENABLE_WORLD_EVENTS
                        var evtCmpt2 = op.Pool.GetExistedItemById (op.ComponentId);
                        for (int ii = 0, iiMax = _eventListeners.Count; ii < iiMax; ii++) {
        ...
        [System.Runtime.CompilerServices.MethodImpl(System.Runtime.CompilerServices.MethodImplOptions.AggressiveInlining)]
#endif
        public T GetFilter<T>() where T : EntityFilter
        {
        ...
        [System.Runtime.CompilerServices.MethodImpl(System.Runtime.CompilerServices.MethodImplOptions.AggressiveInlining)]
#endif
        public EntityFilter GetFilter(Type filterType)
        {
#if DEBUG
            if (filterType == null) { throw new Exception("FilterType is null"); }
            if (!filterType.IsSubclassOf(typeof(EntityFilter))) { throw new Exception(string.Format("Invalid filter-type: {0}", filterType)); }
#endif
            ...
            var filter = (EntityFilter)Activator.CreateInstance(filterType, true);
            filter.Supervisor = this;
#if DEBUG
            for (int j = 0, jMax = _filtersCount; j < jMax; j++)
            {
                if (_filters[j].IncludeMask.IsEquals(filter.IncludeMask) && _filters[j].ExcludeMask.IsEquals(filter.ExcludeMask) && _filters[j].EntityTypeIndex == filter.EntityTypeIndex)
                {
        ...
        /// </summary>
        public void RemoveFilter<T>() where T : EntityFilter
        {
        ...
        [System.Runtime.CompilerServices.MethodImpl(System.Runtime.CompilerServices.MethodImplOptions.AggressiveInlining)]
#endif
        protected void UpdateFilters(int entity, int entityTypeIndex, ComponentMask oldMask, ComponentMask newMask)
        {
            for (int i = 0, iMax = _filtersCount; i < iMax; i++)
            {
                var filter = _filters[i];
                if (!filter.IsEntityTypeCompatible(entityTypeIndex)) continue;
        ...
        protected void UpdateOwners(int entity, bool isRemove)
        {
            var entityData = _entities[entity];
            var owner = _owners[entityData.PipelineContext];
            if (isRemove)
            {
#if DEBUG
                var ii = owner._entitiesCount - 1;
                for (; ii >= 0; ii--)
                {
                    if (owner.Entities[ii] == entity)
                    {
                        break;
                    }
                }
                if (ii == -1) { throw new Exception(string.Format("Something wrong - entity {0} should be in owner {1}, but not exits.", entity, owner)); }
#endif
                owner.RaiseOnRemoveEvent(entity);
            }
            else
            {
                owner.RaiseOnAddEvent(entity);
            }
        }
        ...
        [System.Runtime.InteropServices.StructLayout(System.Runtime.InteropServices.LayoutKind.Sequential, Pack = 1)]
        protected sealed class EntityInternal
        {
            public readonly ComponentMask Mask = new ComponentMask();
